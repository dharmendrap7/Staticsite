    ...
    public class AccountController : BaseController
    {
        ...
        public ActionResult ProfileDeactivate()
        {
            try
            {
                ...
                var user = UserManager1.FindById(userid);
                if (user.UserActive == true || user.UserActive == null)
                {
                    //var data = new ApplicationUser { UserActive = false,Email=user.Email };
                   user.UserActive=false;
                    TempData["msg"] = "Your Account have been deactivated,Login at any time to reactivate";
             var result=UserManager1.Update(user);
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(LoginRegistration model)
        {
            if (!ModelState.IsValid)
            {
                TempData["register"] = 1;
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.register.UserName, Email = model.register.Email, Usertype = (int)model.register.UserType };
                var result = await UserManager1.CreateAsync(user, model.register.Password);
                if (result.Succeeded)
                {
                    ...
                    string code = await UserManager1.GenerateEmailConfirmationTokenAsync(user.Id);
                    var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                    //await UserManager1.SendEmailAsync(user.Id, "Confirm your account", "Please confirm your account by clicking <a href=\"" + callbackUrl + "\">here</a>");
                    MailMessage mail = new MailMessage();
                    mail.To.Add(model.register.Email);
                    mail.From = new MailAddress("premalth01@gmail.com");
                    mail.Subject = "Regarding Email Confirmation"; ;
                    mail.Body = "Please Confirm your email by clicking  <a href=\"" + callbackUrl + "\">here</a>";
                    ...
                    smtp.Send(mail);
                    return RedirectToAction("Confirm", "Account", new { Email = model.register.Email });
                };
                AddErrors(result);
            }
